...
![TGAM1 board underside supplied by the project owner](assets/tgam1_board_bottom_user_photo.jpg)
...
A healthy neural connection must provide valid ThinkGear packets with the 512-sample/s RAW stream enabled. The eight TGAM EEG-band summaries and eSense values are monitor-only telemetry and are never used as classifier input. RAW freshness/rate/activity, packet integrity and the technical `POOR_SIGNAL` contact value participate in one global capture-quality rule used by training, validation, communication, cursor control, command tests and the final Diagnostics capture check.
