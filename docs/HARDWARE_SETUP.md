...
A healthy RAW connection should provide valid ThinkGear packets, RAW 16-bit samples at approximately 512 samples/s, and the standard summary rows when the module publishes them. The application can continue deriving relative `RAW*` continuity metrics when native Attention/Meditation or EEG-band summary rows are unavailable, but BCI training and validation are based directly on RAW epochs.
