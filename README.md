...
The complete mathematical, calibration, validation and persistence contract is documented in [docs/MODEL_STRATEGY.md](docs/MODEL_STRATEGY.md).
...
Model/session schemas are strict Version 1 contracts. Incompatible schemas, algorithms, feature definitions, training/validation fingerprints and malformed/non-finite values are rejected rather than adapted. BCI acquisition also requires a complete 1024-sample RAW window plus a sane live RAW arrival rate/spread before features can be captured.

## Backend audit

Version 1 underwent a backend audit covering acquisition concurrency, bounded buffering, ThinkGear framing, Bluetooth probe isolation, fixed RAW epoch capture, calibration scheduling, model/validation integrity, persistence consistency, user isolation and replay input validation.

The complete findings, corrections, verification scope and physical-hardware limitations are in [AUDIT.md](AUDIT.md).
