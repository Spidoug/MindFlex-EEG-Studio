...
class TransportReader:
    ...
    def connect(self, endpoint: EndpointInfo, baudrate: int = 57600) -> None:
        if int(baudrate) != 57600:
            raise ValueError("MindFlex EEG Studio is configured for RAW at 57600 bps only")
        ...
        try:
            backend.connect(endpoint, 57600)
        except Exception:
            with self._lock:
