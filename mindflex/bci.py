...
from .signal_processing import preprocess_eeg
from .settings import app_data_dir
from .storage import atomic_write_json, read_json_object

# BCI features use the eight canonical TGAM band powers. They may come directly
# from a TGAM summary row or from the locally filtered RAW estimator, keeping
# calibration available in both valid headset output modes.
FEATURE_NAMES = (
...
MODEL_SCHEMA = 1
SESSION_SCHEMA = 1
FEATURE_SCHEMA = "mindflex-eeg-band18-v1"
...
class FeatureExtractor:
    ...
    @classmethod
    def from_bands(cls, bands: dict[str, int | float]) -> np.ndarray:
        """Build the canonical BCI vector from one complete TGAM band update."""
        if not isinstance(bands, dict) or set(bands) != set(EEG_BANDS):
            raise ValueError("A complete eight-band EEG update is required")
        values: dict[str, float] = {}
        for name in EEG_BANDS:
            value = bands[name]
            if isinstance(value, bool) or not isinstance(value, (int, float)):
                raise ValueError("EEG band powers must be numeric")
            parsed = float(value)
            if not math.isfinite(parsed) or parsed < 0.0:
                raise ValueError("EEG band powers must be finite and non-negative")
            values[name] = parsed

        total = sum(values.values())
        if total <= 0.0 or not math.isfinite(total):
            raise ValueError("EEG band update has no usable spectral energy")
        eps = max(1e-12, total * 1e-12)
        theta = values["theta"]
        alpha = values["low_alpha"] + values["high_alpha"]
        beta = values["low_beta"] + values["high_beta"]
        gamma = values["low_gamma"] + values["mid_gamma"]
        probabilities = np.asarray([values[name] / total for name in EEG_BANDS], dtype=np.float64)
        positive = probabilities[probabilities > 0.0]
        entropy = float(-np.sum(positive * np.log(positive))) / math.log(len(EEG_BANDS))
        centers = np.asarray([2.25, 6.0, 9.0, 11.5, 16.5, 25.0, 35.0, 45.0], dtype=np.float64)
        centroid = float(np.sum(centers * probabilities)) / 50.0
        engagement = (beta + gamma + eps) / (theta + alpha + eps)
        vector = np.asarray(
            [
                *[math.log1p(values[name]) for name in EEG_BANDS],
                theta / total,
                alpha / total,
                beta / total,
                gamma / total,
...
class CentroidModel:
    ...
    def from_dict(cls, payload: dict) -> "CentroidModel":
        expected_top = {
            "schema", "algorithm", "feature_schema",
            "feature_names", "labels", "mean", "scale", "centroids", "training_fingerprint",
        }
        ...
        if not isinstance(payload.get("feature_schema"), str) or payload.get("feature_schema") != FEATURE_SCHEMA:
            raise ValueError("Model feature format does not match this application")
...
class ModelStore:
    ...
    def load(self, name: str) -> CentroidModel:
        ...
        return CentroidModel.from_dict(model_payload)
