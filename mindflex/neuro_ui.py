...
from .bci import (
    BCIModel, FeatureExtractor, FeatureFrame, PredictionStabilizer, TrialDecisionAccumulator,
    bci_window, latest_bci_window, validate_model,
)
from .calibration import CALIBRATION_PROTOCOLS, CalibrationEngine, CalibrationPhase
from .controller import EEGController
from .lab import RECORDING_EXTENSION, SessionRecorder, analyze_bci_recording, analyze_replay, run_experiment
from .mental_text import StableInterpreter
from .neural_visual import COMMAND_IDS, COMMAND_SYMBOLS, CommandTestPhase, CommandTestSession, CursorArena
...
class NeuroControlView(ttk.Frame):
    ...
    def __init__(
        ...
        self._calibration_recorder: SessionRecorder | None = None
        self._test_recorder: SessionRecorder | None = None
        self._last_feature_key: tuple[int, int] | None = None
        # One cue boundary is shared by every cue-driven BCI activity. The
        # boundary is captured only after the symbol has been rendered.
        self._bci_boundary_sample: int | None = None
        self._bci_boundary_token = ""
        ...
        self._concentration_filter = PredictionStabilizer(self.runtime.labels_for("concentration"))
        self._cursor_filter = PredictionStabilizer(COMMAND_IDS)
        self._command_log: list[str] = []
        self._last_calibration_visual = None
        self._validation_preview: TrialDecisionAccumulator | None = None
    ...
    def _reset_decisions(self) -> None:
        self._concentration_filter.reset()
        self._cursor_filter.reset()

    def _set_bci_boundary(self, token: str) -> int:
        """Mark the RAW sample that follows a visibly rendered cue."""
        self.update_idletasks()
        sample = self.controller.snapshot().raw_total_samples
        self._bci_boundary_sample = sample
        self._bci_boundary_token = str(token)
        self._last_feature_key = None
        self._reset_decisions()
        return sample

    def _clear_bci_boundary(self, *, reset_decisions: bool = False) -> None:
        self._bci_boundary_sample = None
        self._bci_boundary_token = ""
        self._last_feature_key = None
        if reset_decisions:
            self._reset_decisions()
    ...
    def _feature_frame(
        self,
        *,
        boundary_sample: int | None = None,
        epoch_index: int | None = None,
        require_new: bool = True,
        strict_fixed_grid: bool = False,
    ) -> FeatureFrame | None:
        """Extract exactly one canonical RAW frame on the global BCI grid.

        ``None`` means that the requested window has not arrived yet (or, for
        live mode, that the current signal gate is closed). During calibration
        a complete but unusable window raises ``ValueError`` so the scheduler
        rejects that exact grid index and advances instead of retrying stale EEG.
        """
        snap = self.controller.snapshot()
        boundary = 0 if boundary_sample is None else int(boundary_sample)
        try:
            if epoch_index is None:
                window = latest_bci_window(snap.raw_total_samples, boundary)
            else:
                window = bci_window(boundary, int(epoch_index))
                if window.end_sample > snap.raw_total_samples:
                    return None
        except ValueError:
            if strict_fixed_grid:
                raise
            return None
        if window is None:
            return None
        if not self.rules.feature_snapshot_usable(snap):
            if strict_fixed_grid:
                raise ValueError("Complete BCI window failed the global signal-quality gate")
            return None
        key = (window.start_sample, window.end_sample)
        if require_new and key == self._last_feature_key:
            return None
        samples = self.controller.raw_slice(window.start_sample, window.end_sample)
        if len(samples) != FeatureExtractor.MIN_SAMPLES:
            if strict_fixed_grid:
                raise ValueError("Complete BCI window is no longer present in the RAW buffer")
            return None
        try:
            vector = FeatureExtractor.from_raw(samples)
        except (ValueError, ArithmeticError):
            if strict_fixed_grid:
                raise
            return None
        self._last_feature_key = key
        return FeatureFrame(
    ...
    def runtime_state(self) -> RuntimeState:
        snap = self.controller.snapshot()
        model_ready, model_validated, live_ready, communication_ready = self.runtime.readiness_flags(self.rules)
        return RuntimeState(
    ...
    def _cancel_live_job(self) -> None:
        ...
        self._finish_test_recording("stopped")
        self._cursor_running = False
        self._cursor_arena.stop()
        self._clear_bci_boundary(reset_decisions=True)
    ...
    def _recording_path(self, prefix: str) -> Path:
        recordings_dir = self.runtime.model_store.root / "recordings"
        recordings_dir.mkdir(parents=True, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        base = recordings_dir / f"{prefix}-{stamp}{RECORDING_EXTENSION}"
        suffix = 1
        path = base
        while path.exists():
            path = recordings_dir / f"{prefix}-{stamp}-{suffix:02d}{RECORDING_EXTENSION}"
            suffix += 1
        return path

    def _start_calibration_recording(self, profile: str, purpose: str) -> bool:
        if self._calibration_recorder is not None and not self._finish_calibration_recording("replaced"):
            return False
        recorder = SessionRecorder(
            self._recording_path(f"bci-{purpose}-{profile}"),
            session_kind="bci-calibration",
            profile=profile,
            purpose=purpose,
            owner=self.runtime.user_name,
        )
        try:
            recorder.start(self.controller)
            recorder.mark_event("calibration_start", profile=profile, purpose=purpose, phase="prepare")
        except (OSError, ValueError, RuntimeError) as exc:
            self.status_var.set(self._t("lab.recording.error", error=str(exc)))
            return False
        self._calibration_recorder = recorder
        return True

    def _finish_calibration_recording(self, status: str) -> bool:
        recorder, self._calibration_recorder = self._calibration_recorder, None
        if recorder is None:
            return False
        try:
            recorder.mark_event("calibration_end", phase=status)
            path = recorder.stop(status=status)
        except (OSError, ValueError, RuntimeError) as exc:
            self.status_var.set(self._t("lab.recording.error", error=str(exc)))
            return False
        if path is None or recorder.error:
            self.status_var.set(self._t("lab.recording.error", error=recorder.error or "recording was not saved"))
            return False
        return True

    def _rollback_calibration_run(self, profile: str, purpose: str) -> None:
        if not profile:
            return
        target = self.runtime.validation_sessions[profile] if purpose == "validation" else self.runtime.sessions[profile]
        if purpose == "validation":
            target.clear()
            self.runtime.validations.pop(profile, None)
        else:
            del target.samples[self._calibration_start_sample_count :]
        self._persist_profile_safely(profile)

    def _start_test_recording(self) -> bool:
        if self._test_recorder is not None and not self._finish_test_recording("replaced"):
            return False
        recorder = SessionRecorder(
            self._recording_path("bci-test-cursor"),
            session_kind="bci-test",
            profile="cursor",
            purpose="blind-test",
            owner=self.runtime.user_name,
        )
        try:
            recorder.start(self.controller)
            recorder.mark_event("test_start", profile="cursor", purpose="blind-test")
        except (OSError, ValueError, RuntimeError) as exc:
            self.status_var.set(self._t("lab.recording.error", error=str(exc)))
            return False
    ...
    def _finish_test_recording(self, status: str) -> bool:
        recorder, self._test_recorder = self._test_recorder, None
        if recorder is None:
            return True
        try:
            recorder.mark_event("test_end", phase=status)
            path = recorder.stop(status=status)
        except (OSError, ValueError, RuntimeError) as exc:
            self.status_var.set(self._t("lab.recording.error", error=str(exc)))
            return False
    ...
    def _start_calibration(self, profile: str, labels, *, purpose: str) -> None:
        ...
        if purpose == "validation":
            protocol = protocol.for_validation()
            target = self.runtime.validation_sessions[profile]
            target.clear()
            self.runtime.validations.pop(profile, None)
            if not self._persist_profile_safely(profile):
                return
        else:
            target = self.runtime.sessions[profile]

        self._clear_bci_boundary(reset_decisions=True)
        model_for_preview = self.runtime.models.get(profile) if purpose == "validation" else None
        self._validation_preview = (
            TrialDecisionAccumulator(model_for_preview.labels)
            if model_for_preview is not None and model_for_preview.ready
            else None
        )

        def feature_provider(epoch_index: int):
            if self._bci_boundary_sample is None:
                return None
            frame = self._feature_frame(
                boundary_sample=self._bci_boundary_sample,
                epoch_index=epoch_index,
                require_new=False,
                strict_fixed_grid=True,
            )
            if frame is None:
                return None
            return list(frame.features), frame.start_sample, frame.end_sample
        ...
        def epoch_sink(
            ...
            if self._calibration_recorder is not None:
                self._calibration_recorder.mark_event(
                    "epoch_accepted",
                    raw_sample=raw_start,
                    profile=profile,
                    purpose=purpose,
                    label=label,
                    trial_id=trial_id,
                    phase=str(epoch_index),
                )
            if purpose == "validation":
                model = self.runtime.models.get(profile)
                if model and model.ready and self._validation_preview is not None:
                    try:
                        stable = self._validation_preview.push(model.predict(vector))
                    except (RuntimeError, ValueError, ArithmeticError):
                        stable = None
                    if stable is not None:
                        self._update_validation_prediction(stable)

        if not self._start_calibration_recording(profile, purpose):
            return
    ...
    def _calibration_tick(self) -> None:
        ...
        if engine is None:
            return
        if self._calibration_recorder is not None and self._calibration_recorder.error:
            error = self._calibration_recorder.error
            self._cancel_calibration()
            self.status_var.set(self._t("lab.recording.error", error=error))
            return
        previous_phase = engine.phase
        progress = engine.tick()
        self._update_calibration_progress(progress)
        if progress.phase == CalibrationPhase.TASK and previous_phase != CalibrationPhase.TASK:
            trial = engine.current
            token = f"calibration:{trial.trial_id if trial else progress.trial_number}"
            boundary = self._set_bci_boundary(token)
            if trial is not None and self._calibration_recorder is not None:
                self._calibration_recorder.mark_event(
                    "trial_start",
                    raw_sample=boundary,
            ...
            if self._validation_preview is not None:
                self._validation_preview.reset()
            engine.mark_task_visible()
        elif progress.phase != CalibrationPhase.TASK and previous_phase == CalibrationPhase.TASK:
            trial = engine.current
            if trial is not None and self._calibration_recorder is not None:
                self._calibration_recorder.mark_event(
                    "trial_end",
                    raw_sample=self.controller.snapshot().raw_total_samples,
                    profile=self._calibration_profile,
                    purpose=self._calibration_purpose,
                    label=trial.label,
                    trial_id=trial.trial_id,
        ...
        if progress.phase == CalibrationPhase.COMPLETE:
            ...
            recording_ok = self._finish_calibration_recording("complete")
            self._calibration_engine = None
            self._calibration_profile = ""
            self._calibration_purpose = ""
            self._validation_preview = None
            self._clear_bci_boundary(reset_decisions=True)
            if not recording_ok:
                self._rollback_calibration_run(profile, purpose)
    ...
    def _cancel_calibration(self) -> None:
        ...
        if self._calibration_engine is not None:
            ...
            self._finish_calibration_recording("cancelled")
            self._rollback_calibration_run(profile, purpose)
            self._calibration_engine = None
            self._calibration_profile = ""
            self._calibration_purpose = ""
    ...
    def _finalize_training(self, profile: str, *, collected: int = 0, rejected: int = 0, refresh: bool = True) -> None:
        ...
        if self.rules.training_ready(session, labels):
            try:
                self.runtime.models[profile] = BCIModel.train(session)
    ...
    def _build_validation(self) -> None:
        ...
        class_threshold = self.rules.policy.minimum_validation_accuracy_per_class
        decision_threshold = self.rules.policy.minimum_validation_decision_rate
        minimum_trials = self.rules.policy.minimum_validation_trials_per_class
            ...
            if result:
                ...
                ttk.Label(
                    ...
                    text=self._t(
                        ...
                        balanced=result.balanced_accuracy * 100,
                        decision=result.decision_rate * 100,
                ...
                status_key = "validation.passed" if self.rules.validation_passed(result, model.labels) else "validation.failed"
                ttk.Label(
                    controls,
                    text=self._t(
                        status_key,
                        threshold=threshold * 100,
                        class_threshold=class_threshold * 100,
                        decision_threshold=decision_threshold * 100,
    ...
    def _run_validation(self, profile: str) -> None:
        ...
        self.status_var.set(
            self._t(
                "validation.passed" if self.rules.validation_passed(result, model.labels) else "validation.failed",
                threshold=threshold * 100,
                class_threshold=class_threshold * 100,
                decision_threshold=decision_threshold * 100,
                trials=self.rules.policy.minimum_validation_trials_per_class,
            )
        )
        self._persist_profile_safely(profile)
        self._refresh_after_state_change()
    ...
    def _cursor_model_ready(self) -> bool:
        return self.runtime.validated("cursor", self.rules)
    ...
    def _start_command_test(self) -> None:
        ...
        if len(labels) < 2:
            self.status_var.set(self._t("live.not_validated"))
            return
        if not self._start_test_recording():
            return
    ...
    def _schedule_live(self) -> None:
        ...
        snap = self.controller.snapshot()
        test_snap = None
        entered_new_cue = False

        # Cue state always advances before signal extraction. When a new cue is
        # rendered, that visible event becomes sample zero for every BCI window
        # used by the test.
        if self._test_recorder is not None and self._test_recorder.error:
            error = self._test_recorder.error
            self._command_test.stop()
            self._finish_test_recording("error")
            self.status_var.set(self._t("lab.recording.error", error=error))
        if self._command_test.running:
            previous_phase = self._command_test.phase
            previous_cue = self._command_test.current_cue
            test_snap = self._command_test.tick(now)
            entered_new_cue = (
                test_snap.phase == CommandTestPhase.CUE
                and (previous_phase != CommandTestPhase.CUE or previous_cue != test_snap.cue)
            )
            if entered_new_cue:
                self._refresh_command_test_visual()
                boundary = self._set_bci_boundary(f"command-test:{test_snap.trial_number}:{test_snap.cue}")
                if self._test_recorder is not None:
                    self._test_recorder.mark_event(
                        "trial_start", raw_sample=boundary, profile="cursor", purpose="blind-test",
                        label=test_snap.cue, trial_id=f"test-{test_snap.trial_number:04d}", phase="cue"
                    )
                self._command_test.mark_cue_visible()
                snap = self.controller.snapshot()
            elif previous_phase == CommandTestPhase.CUE and test_snap.phase != CommandTestPhase.CUE:
                if self._test_recorder is not None:
                    self._test_recorder.mark_event(
                        "trial_end", raw_sample=self.controller.snapshot().raw_total_samples,
                        profile="cursor", purpose="blind-test", label=previous_cue,
                        trial_id=f"test-{test_snap.trial_number:04d}", phase="cue"
                    )
                self._clear_bci_boundary(reset_decisions=True)
            if test_snap.phase == CommandTestPhase.COMPLETE and self._test_recorder is not None:
                self._finish_test_recording("complete")

        cue_aligned = test_snap is not None and test_snap.phase == CommandTestPhase.CUE
        boundary = self._bci_boundary_sample if cue_aligned else None
        frame = self._feature_frame(boundary_sample=boundary) if self.rules.feature_snapshot_usable(snap) else None

        concentration_prediction = None
        cursor_prediction = None
        if frame is not None:
            try:
                if self.runtime.validated("concentration", self.rules):
                    concentration_prediction = self.runtime.models["concentration"].predict(frame.features)
                if self.runtime.validated("cursor", self.rules):
                    cursor_prediction = self.runtime.models["cursor"].predict(frame.features)
            except (RuntimeError, ValueError, ArithmeticError) as exc:
                self.status_var.set(self._t("model.error", error=str(exc)))

        if not snap.receiving:
            self._reset_decisions()
            stable_concentration = None
            stable_cursor = None
        else:
            stable_concentration = self._concentration_filter.push(concentration_prediction)
            stable_cursor = self._cursor_filter.push(cursor_prediction)

        if self.runtime.validated("concentration", self.rules):
            if stable_concentration is None:
                self.trigger_value.configure(text="—")
                self.trigger_conf.configure(text=self._t("neural.visual.no_prediction"))
            else:
                display = self._label_display(stable_concentration.label)
                self.trigger_value.configure(text=display)
                self.trigger_conf.configure(
                    text=self._t("live.confidence", value=stable_concentration.confidence * 100)
                )
        else:
            self.trigger_value.configure(text="—")
            self.trigger_conf.configure(text=self._t("live.not_validated"))

        self._update_cursor_prediction_visuals(stable_cursor)

        if self._command_test.running and test_snap is not None:
            if test_snap.phase == CommandTestPhase.CUE:
                self._command_test.observe(cursor_prediction)
            if not entered_new_cue:
                self._refresh_command_test_visual()

        if self._cursor_running:
            dt = max(0.01, min(0.20, now - self._cursor_last_tick))
            self._cursor_last_tick = now
            label = stable_cursor.label if stable_cursor is not None else None
            confidence = stable_cursor.confidence if stable_cursor is not None else 0.0
            self._cursor_arena.step(label, confidence, dt)
            ...
            if stable_cursor is None:
                ...
                self.cursor_prediction_label.configure(
                    text=self._t(
                        ...
                        symbol=COMMAND_SYMBOLS.get(stable_cursor.label, "•"),
                        label=self._label_display(stable_cursor.label),
                        confidence=stable_cursor.confidence * 100,
                    )
                )
            self._draw_cursor_arena()

        self._live_job = self.after(100, self._schedule_live)
    ...
    def _toggle_communication(self) -> None:
        if self._communication_running:
            ...
            return
        if not self.runtime.validated("communication", self.rules):
            self.status_var.set(self._t("live.not_validated"))
            return
        if not self.controller.snapshot().receiving:
            self.status_var.set(self._t("stream.required"))
    ...
    def _communication_tick(self) -> None:
        ...
        try:
            ...
            if frame is None:
                self._live_job = self.after(120, self._communication_tick)
                return
            prediction = self.runtime.models["communication"].predict(frame.features)
        except (RuntimeError, ValueError, ArithmeticError) as exc:
            self._communication_running = False
            self.communication_button.configure(text=self._t("communication.start"))
            self.status_var.set(self._t("model.error", error=str(exc)))
    ...
    def _build_laboratory(self) -> None:
        ...
        replay_actions = ttk.Frame(replay, style="Card.TFrame")
        replay_actions.pack(fill="x")
        ttk.Button(replay_actions, text=self._t("lab.choose_file"), command=self._choose_replay).pack(side="left")
        self.replay_analyze_button = ttk.Button(
            replay_actions, text=self._t("lab.analyze_bci"), command=self._analyze_replay_bci, state="disabled"
        )
        self.replay_analyze_button.pack(side="left", padx=(6, 0))
        self.replay_summary_var = tk.StringVar(value=self._t("lab.replay.none"))
        ttk.Label(replay, textvariable=self.replay_summary_var, style="CardMeta.TLabel", wraplength=620, justify="left").pack(anchor="w", pady=(10, 0))
        ...
        self.lab_result_var = tk.StringVar(value=self._t("lab.result.none"))
        ttk.Label(
            experiments, textvariable=self.lab_result_var, style="CardMeta.TLabel",
    ...
    def _choose_replay(self) -> None:
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[("MindFlex Session", f"*{RECORDING_EXTENSION}"), (self._t("file.all"), "*.*")],
        )
        if not path:
            return
        ...
        self.replay_summary_var.set(
            self._t(
                "lab.replay.summary",
                raw=summary.raw_samples,
                duration=summary.duration_seconds,
                frames=summary.telemetry_frames,
                ...
                rate=summary.mean_raw_rate_hz,
                checksums=summary.bad_checksums,
                dropped=summary.dropped_bytes,
                profile=summary.profile or "—",
                purpose=summary.purpose or "—",
            )
        )
        if hasattr(self, "replay_analyze_button"):
            self.replay_analyze_button.configure(state="normal" if summary.trial_count >= 2 else "disabled")
        self.status_var.set(self._t("lab.replay.loaded", path=path))

    def _analyze_replay_bci(self) -> None:
        if self._replay_path is None:
            return
        try:
            report = analyze_bci_recording(self._replay_path)
        except (OSError, ValueError, RuntimeError, ArithmeticError) as exc:
            self.status_var.set(self._t("lab.replay.error", error=str(exc)))
            return
        result = report.experiment
        weakest = result.weakest_label or "—"
        weakest_value = result.per_label.get(weakest, 0.0) * 100 if result.per_label else 0.0
        self.status_var.set(
            self._t(
                "lab.offline.result",
                ...
                rejected=report.rejected_epochs,
                accuracy=result.accuracy * 100,
                balanced=result.balanced_accuracy * 100,
                decision=result.decision_rate * 100,
                weakest=weakest,
        ...
        if hasattr(self, "lab_result_var"):
            self.lab_result_var.set(self._format_experiment_details(result))

    def _format_experiment_details(self, result) -> str:
        lines = [
            self._t(
                "lab.metrics",
                accuracy=result.accuracy * 100,
        ...
        if result.per_label:
            class_rows = []
            for label, value in sorted(result.per_label.items()):
                class_rows.append(f"{self._label_display(label)} {value * 100:.1f}%")
            lines.append(self._t("lab.per_class", values=" · ".join(class_rows)))
        if result.confusion:
            confusion_rows = []
            for expected, row in sorted(result.confusion.items()):
                pieces = []
                for predicted, count in sorted(row.items()):
                    shown = self._t("lab.no_decision") if predicted == "<no-decision>" else self._label_display(predicted)
                    pieces.append(f"{shown}:{count}")
                confusion_rows.append(f"{self._label_display(expected)} → " + ", ".join(pieces))
            lines.append(self._t("lab.confusion", values=" | ".join(confusion_rows)))
        return "\n".join(lines)
