...
class NeuroControlView(ttk.Frame):
    ...
    def __init__(
        ...
        self._last_feature_key: tuple[str, int] | None = None
    ...
    def _feature_vector(self, seconds: float = 2.0, *, require_new: bool = True):
        snap = self.controller.snapshot()
        if not self.rules.feature_snapshot_usable(snap):
            return None
        complete_bands = snap.bands_age <= self.rules.policy.metric_stale_after_seconds and len(snap.bands) == 8
        if complete_bands:
            key = ("bands", snap.bands_updates)
            if require_new and key == self._last_feature_key:
                return None
            try:
                vector = FeatureExtractor.from_bands(snap.bands)
        ...
        required = max(FeatureExtractor.MIN_SAMPLES, int(round(FeatureExtractor.SAMPLE_RATE * float(seconds))))
        if snap.raw_age > 2.0 or snap.raw_buffered_samples < required:
            return None
        # One prediction per 128 new RAW samples (4 Hz at the TGAM rate) avoids
        # repeatedly classifying the same overlapping buffer on fast UI ticks.
        key = ("raw", snap.raw_total_samples // 128)
        if require_new and key == self._last_feature_key:
            return None
        samples = self.controller.raw_values(seconds, max_samples=required)
        if len(samples) < required:
            return None
        try:
            vector = FeatureExtractor.from_raw(
                samples,
                sample_rate=FeatureExtractor.SAMPLE_RATE,
    ...
    def _start_calibration(self, profile: str, labels, *, purpose: str) -> None:
        ...
        def epoch_sink(label: str, vector: list[float], trial_id: str, epoch_index: int, epoch_seconds: float) -> None:
            target.add(
                label,
                vector,
                timestamp=time.time(),
    ...
    def _schedule_live(self) -> None:
        ...
        if self._command_test.running:
            ...
            if test_snap.phase == CommandTestPhase.CUE and cursor_prediction is not None:
