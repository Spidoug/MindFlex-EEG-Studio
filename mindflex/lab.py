...
def _stratified_holdout(session: TrainingSession, fraction: float = 0.25) -> tuple[TrainingSession, TrainingSession]:
    ...
    if not math.isfinite(holdout_fraction) or not 0.0 < holdout_fraction < 1.0:
        raise ValueError("Holdout fraction must be finite and between 0 and 1")
    ...
    for label in sorted(grouped):
        ...
        holdout_count = max(1, min(len(trial_ids) - 1, round(len(trial_ids) * holdout_fraction)))
...
def replay_csv(path: Path) -> Iterator[dict[str, float | int | None]]:
    with path.open("r", newline="", encoding="utf-8") as handle:
        ...
        if extra:
            raise ValueError(f"Replay CSV contains unexpected fields: {', '.join(sorted(extra))}")
        for row in reader:
            if None in row:
            ...
            for key in SessionRecorder.FIELDNAMES:
                ...
                if value in (None, "", "None"):
                    ...
                    if not parsed.is_integer():
                        raise ValueError(f"Replay CSV contains a non-integer value in {key}")
                    integer = int(parsed)
                    if key in {"attention", "meditation"} and not 0 <= integer <= 100:
                        raise ValueError(f"Replay CSV contains an out-of-range value in {key}")
                    if key == "blink" and not 0 <= integer <= 255:
                        raise ValueError("Replay CSV contains an out-of-range blink value")
                    if key in EEG_BANDS and integer < 0:
                        raise ValueError(f"Replay CSV contains a negative value in {key}")
                    clean[key] = integer
            yield clean
