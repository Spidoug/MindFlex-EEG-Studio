...
class BluetoothWorker:
        ...
        try:
            ...
            if pending:
                try:
                    ...
                    LOGGER.exception("Bluetooth worker could not finish pending tasks cleanly")
            try:
                self.loop.run_until_complete(self.loop.shutdown_asyncgens())
            except Exception:
                LOGGER.exception("Bluetooth worker async-generator shutdown failed")
            self.loop.close()
    ...
    def _close_service(service) -> None:
        ...
        try:
            ...
            LOGGER.debug("RFCOMM service close failed", exc_info=True)
    ...
    async def _probe(self) -> tuple[bool, str]:
        ...
        if ok:
            # Only a confirmed ThinkGear channel may enter the canonical EEG
            # pipeline. Failed RFCOMM probes are discarded completely.
    ...
    async def _close_channel(self, *, close_service: bool = True) -> None:
        if self.reader is not None:
            try:
                ...
                LOGGER.debug("Bluetooth DataReader detach failed", exc_info=True)
            try:
                self.reader.close()
            except Exception:
                LOGGER.debug("Bluetooth DataReader close failed", exc_info=True)
            self.reader = None
        if self.socket is not None:
            try:
                self.socket.close()
            except Exception:
                LOGGER.debug("Bluetooth StreamSocket close failed", exc_info=True)
            self.socket = None
    ...
    async def connect(self, device_id: str) -> None:
        ...
        try:
            ...
            LOGGER.exception("Bluetooth on_connecting callback failed")
            self._pending_endpoint = None
            try:
                self.on_error(exc)
            except Exception:
                LOGGER.exception("Bluetooth error callback failed")
            ...
            for round_index in range(self.DISCOVERY_ROUNDS):
                ...
                if self.bt_device is not None:
                    try:
                        ...
                        LOGGER.debug("BluetoothDevice close after discovery failed", exc_info=True)
                    self.bt_device = None
    ...
    async def disconnect_internal(self, notify: bool = True, from_receive: bool = False) -> None:
        ...
        if self.bt_device is not None:
            try:
                ...
                LOGGER.debug("BluetoothDevice close failed", exc_info=True)
            self.bt_device = None
        ...
        if was_connected:
            try:
                ...
                LOGGER.exception("Bluetooth on_disconnected callback failed")
        if notify:
    ...
    def close(self) -> None:
        ...
        if loop is not None and not loop.is_closed():
            try:
                ...
                LOGGER.warning("Bluetooth worker disconnect during close failed", exc_info=True)
            try:
