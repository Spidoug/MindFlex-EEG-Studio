    ...
    def update(
        ...
        *,
        ...
        if ylim is None and y_arr.size:
            ...
            if finite.size:
                if robust_ylim and finite.size >= 20:
                    lo, hi = (float(value) for value in np.percentile(finite, (2.0, 98.0)))
                else:
                ...
                ylim = (lo - pad, hi + pad)
                if robust_ylim:
                    display_y = np.clip(y_arr, ylim[0], ylim[1])
        self.line.set_data(x_arr, display_y)
