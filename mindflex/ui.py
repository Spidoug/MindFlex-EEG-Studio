...
class MonitorView(ttk.Frame):
    def __init__(self, master, app: "MindFlexApp") -> None:
        ...
        self.metric_vars = {name: tk.StringVar(value="—") for name in ("raw_rate", "attention", "meditation", "packets")}
    ...
    def _build(self) -> None:
        ...
        metrics = ResponsiveCardGrid(host, min_card_width=155, max_columns=4, gap=10)
    ...
    def _render_graphs(self) -> None:
        ...
        if x:
            self.raw_plot.update(x, y, xlim=(-seconds, 0.0))
        data = {}
...
class MindFlexApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.user_profile = self._ask_user_profile()
    ...
    def _ask_user_profile(self) -> UserProfile:
        while True:
    ...
    def runtime_state(self) -> RuntimeState:
        ...
        return RuntimeState(
            ...
            model_ready=self.neuro_runtime.model_ready,
            model_validated=self.neuro_runtime.any_validated(threshold),
            live_ready=self.neuro_runtime.live_ready(threshold),
            communication_ready=self.neuro_runtime.communication_ready(threshold),
        )
