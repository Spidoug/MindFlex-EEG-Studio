...
class MonitorView(ttk.Frame):
    ...
    def _toggle_recording(self) -> None:
        ...
        if not self.app.controller.snapshot().receiving:
            ...
            return
        recordings_dir = self.app.user_profile.data_dir / "recordings"
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        path = recordings_dir / f"monitor-{stamp}.mfs"
        suffix = 1
        while path.exists():
            path = recordings_dir / f"monitor-{stamp}-{suffix:02d}.mfs"
            suffix += 1
        recorder = SessionRecorder(
            path, session_kind="monitor", owner=self.app.user_profile.full_name
        )
        try:
            recorder.start(self.app.controller)
        ...
        self.recording_var.set(self.t("monitor.recording.active", path=str(path)))
