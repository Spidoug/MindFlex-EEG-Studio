...
from .bci import CentroidModel, ModelStore, TrainingSession, ValidationResult, validate_model
...
VALIDATION_STRATEGY = "independent-trial-v1"
...
class NeuroRuntime:
    ...
    def _restore_metadata(self, profile: str, metadata: dict) -> None:
        ...
        if set(metadata) - allowed:
            LOGGER.warning("Ignoring %s metadata with fields outside the Version 1 contract", profile)
            return
        ...
        if validation is None:
            return
        expected_fields = {
            "schema", "strategy", "total", "correct", "accuracy", "per_label",
            "trials_per_label", "correct_per_label", "model_fingerprint", "validation_fingerprint",
        }
        ...
        result = ValidationResult(
            ...
            correct,
            float(accuracy),
            {key: float(value) for key, value in per_label.items()},
            dict(trials_per_label),
            dict(correct_per_label),
            model_fingerprint,
    ...
    def _model_matches_session(self, profile: str, model: CentroidModel) -> bool:
