...
class CalibrationEngine:
    ...
    must return a fresh feature vector for the requested epoch, or None if the
    current EEG-band update is not usable.
    """
    ...
    def tick(self, now: float | None = None) -> CalibrationProgress:
        ...
        # Capture at most one due update per tick. A delayed GUI callback cannot
        # reconstruct measurements that existed in the past, so missed updates
        # are skipped instead of duplicating the current EEG-band state.
        if (
            ...
            if trial is not None:
                try:
                    ...
                    if vector is None:
                        raise ValueError("EEG band update is not currently usable")
