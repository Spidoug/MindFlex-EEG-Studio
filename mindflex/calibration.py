...
from .settings import BCI_EPOCH_SECONDS, BCI_STEP_SECONDS
...
class CalibrationProtocol:
    """Trial-level protocol only.

    BCI epoch length and step are global Version 1 rules and deliberately do
    not live in protocol presets. Presets may change trial count and phase
    duration, never signal-processing behavior.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.prepare_seconds < 0 or self.task_seconds < BCI_EPOCH_SECONDS or self.rest_min_seconds < 0:
    ...
    @property
    def epochs_per_trial(self) -> int:
        return 1 + int(math.floor((self.task_seconds - BCI_EPOCH_SECONDS) / BCI_STEP_SECONDS + 1e-9))
...
def _canonical_labels(labels: Iterable[str]) -> tuple[str, ...]:
    ...
    for label in labels:
        ...
        clean = label.strip()
        if not clean or clean != label or len(clean) > 80 or any(ord(ch) < 32 for ch in clean):
...
# Provider returns (features, raw_start, raw_end) for one exact fixed-grid index.
FeatureProvider = Callable[[int], tuple[list[float], int, int] | None]
EpochSink = Callable[[str, list[float], str, int, int, int], None]


class CalibrationEngine:
    """Non-blocking trial scheduler shared by training and validation.

    Signal timing is sample-driven. During TASK the engine requests sequential
    fixed-grid RAW epochs. A delayed GUI callback can therefore recover exact
    windows from the controller buffer instead of duplicating a newer frame.
    """
    ...
    def mark_task_visible(self, now: float | None = None) -> None:
        """Align TASK timing to the instant the cue is actually visible."""
        if self.phase != CalibrationPhase.TASK:
            raise RuntimeError("Task visibility can only be marked during TASK")
        stamp = _monotonic_stamp(now)
        self.phase_started = stamp
        self.phase_deadline = stamp + self.protocol.task_seconds
    ...
    def tick(self, now: float | None = None) -> CalibrationProgress:
        ...
        if self.phase == CalibrationPhase.TASK:
            self._collect_available()

        while self.running and stamp >= self.phase_deadline:
            if self.phase == CalibrationPhase.PREPARE:
                # TASK starts at the callback that can actually render the cue.
                self._enter(CalibrationPhase.TASK, stamp, self.protocol.task_seconds)
                break
            if self.phase == CalibrationPhase.TASK:
                rest = self._rng.uniform(self.protocol.rest_min_seconds, self.protocol.rest_max_seconds)
                self._enter(CalibrationPhase.REST, stamp, rest)
                continue
    ...
    def _collect_available(self) -> None:
        trial = self.current
        if trial is None:
            return
        # Catch up exact stored RAW frames after a brief UI delay, but cap work
        # per GUI tick so rendering remains responsive.
        for _ in range(8):
            if self._trial_epoch_index >= self.protocol.epochs_per_trial:
                return
            index = self._trial_epoch_index
            try:
                frame = self.feature_provider(index)
            ...
            if frame is None:
                return
            vector, raw_start, raw_end = frame
            try:
                clean = [float(value) for value in vector]
                self.epoch_sink(trial.label, clean, trial.trial_id, index, int(raw_start), int(raw_end))
            except (ValueError, TypeError, ArithmeticError):
                self.epochs_rejected += 1
            else:
